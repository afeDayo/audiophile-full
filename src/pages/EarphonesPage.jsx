  ...
  return (
    <div>
      ...
      {earphones.map((product) => (
        <div
          key={product._id}
          className="flex flex-col lg:flex-row items-center justify-between px-6 md:px-10 lg:px-12 xl:px-[11.458333vw] mt-16 md:mt-20 lg:mt-40 gap-8 md:gap-12 mb-26 lg:gap-30"
        >
          {/* Product Image */}
          <div className="w-full lg:w-[54%] bg-[#F1F1F1] rounded-lg h-[352px] md:h-[450px] lg:h-[560px] flex items-center justify-center relative">
            <img
              src={product.image}
              alt={product.name}
              className="w-60 md:w-[242px] object-contain lg:w-[296px]"
            />
            <div className="absolute bottom-6 md:bottom-20 left-1/2 -translate-x-1/2 w-[120px] md:w-[200px] h-2 rounded-full bg-black blur-xl md:blur-2xl"></div>
          </div>

          {/* Product Info */}
          <div className="w-full lg:w-[45%] xl:w-[445px] text-center lg:text-start flex flex-col gap-4 md:gap-[29px]">
            {product.isNew && (
              <h4 className="font-normal text-xs md:text-[14px] tracking-[8px] uppercase text-[#D87D4A]">
                NEW PRODUCT
              </h4>
            )}
            <h1 className="font-bold text-[28px] md:text-[40px] lg:text-4xl leading-tight md:leading-11 tracking-[0.5px] md:tracking-[2px] uppercase text-[#000000] md:px-16 lg:px-0">
              {product.name}
            </h1>
            <p className="font-normal text-[15px] leading-relaxed text-[#00000050] px-2 md:px-16 lg:px-0">
              {product.description}
            </p>
